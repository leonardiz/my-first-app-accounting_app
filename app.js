...
function exportInvoiceToPdf(invoice) {
  ...
  const startY = drawPdfDocumentHeader(doc, {
    context,
    title: "Invoice",
    periodLabel: `For the period: Invoice Date ${formatLongDate(invoice.invoiceDate)} to Due Date ${formatLongDate(invoice.dueDate)}`,
  });

  doc.setFont("helvetica", "normal");
  doc.setTextColor(51, 65, 85);
  doc.setFontSize(10);
  doc.text(`Invoice Number: ${invoice.invoiceNumber}`, 40, startY);
  doc.text(`Client: ${invoice.clientName}`, 40, startY + 16);
  doc.text(`Status: ${invoice.status}`, doc.internal.pageSize.getWidth() - 40, startY, { align: "right" });
  ...
  doc.autoTable({
    ...
    margin: { left: 40, right: 40, bottom: 70, top: 56 },
    columnStyles: buildNumericColumnStyles([1, 2, 3]),
    ...buildStandardPdfTableStyles(),
  });

  drawPdfSummaryBox(doc, doc.lastAutoTable.finalY + 22, {
    eyebrow: "Total Due",
    primaryValue: formatPdfCurrency(invoice.totalAmount),
    detailRows: [
      { label: "Subtotal", value: formatPdfCurrency(invoice.subtotal) },
      { label: "Tax", value: `${invoice.taxPercentage}%` },
...
function exportBillToPdf(bill) {
  ...
  const startY = drawPdfDocumentHeader(doc, {
    context,
    title: "Bill",
    periodLabel: `For the period: Bill Date ${formatLongDate(bill.billDate)} to Due Date ${formatLongDate(bill.dueDate)}`,
  });

  doc.setFont("helvetica", "normal");
  doc.setTextColor(51, 65, 85);
  doc.setFontSize(10);
  doc.text(`Bill Number: ${bill.billNumber}`, 40, startY);
  doc.text(`Supplier: ${bill.supplierName}`, 40, startY + 16);
  doc.text(`Status: ${bill.status}`, doc.internal.pageSize.getWidth() - 40, startY, { align: "right" });
  ...
  drawPdfSummaryBox(doc, doc.lastAutoTable.finalY + 22, {
    eyebrow: "Total Bill Amount",
    primaryValue: formatPdfCurrency(bill.totalAmount),
    detailRows: [
      { label: "Subtotal", value: formatPdfCurrency(bill.subtotal) },
      { label: "Tax", value: `${bill.taxPercentage}%` },
      { label: "Due Date", value: formatLongDate(bill.dueDate) },
    ],
  });
  finalizePdfPages(doc);
...
function getPrintableReportPeriod(reportKey) {
  const descriptor = getReportDateRangeDescriptor();
  const periodText =
    descriptor.startDate && descriptor.endDate
      ? `For the period: ${formatLongDate(descriptor.startDate)} to ${formatLongDate(descriptor.endDate)}`
      : descriptor.startDate
        ? `For the period: From ${formatLongDate(descriptor.startDate)}`
        : descriptor.endDate
          ? `For the period: Up to ${formatLongDate(descriptor.endDate)}`
          : "For the period: All Time";
  return {
    label: periodText,
...
function exportSectionToPdf(reportKey) {
  ...
  report.tables.forEach((table) => {
    cursorY = ensurePdfSectionSpace(doc, cursorY, 78);
    ...
    doc.autoTable({
      ...
      margin: { top: 56, right: 40, bottom: 70, left: 40 },
  ...
  if (report.summaryBox) {
    drawPdfSummaryBox(doc, cursorY, report.summaryBox);
  }
...
function buildStandardPdfTableStyles() {
  return {
    styles: {
      font: "helvetica",
      fontSize: 9,
    ...
    didParseCell: (data) => {
      ...
      const isBodyOrFoot = data.section === "body" || data.section === "foot";
      if (isHeader) {
        return;
      }

      const raw = Array.isArray(data.row?.raw) ? data.row.raw[data.column.index] : data.cell.raw;
      if (!isBodyOrFoot || typeof raw !== "string") {
        return;
      }

      if (looksLikePositivePdfAmount(raw)) {
...
function drawPdfDocumentHeader(doc, { context, title, periodLabel }) {
...
function ensurePdfSectionSpace(doc, cursorY, sectionHeight = 80) {
  ...
  if (cursorY + sectionHeight <= pageHeight - 90) {
    return cursorY;
  }
...
function drawPdfSummaryBox(doc, startY, summaryBox) {
  if (!summaryBox) {
    return startY;
  }

  let cursorY = ensurePdfSectionSpace(doc, startY, 124);
  const marginX = 40;
  const pageWidth = doc.internal.pageSize.getWidth();
  const boxWidth = pageWidth - marginX * 2;
  const detailRows = Array.isArray(summaryBox.detailRows) ? summaryBox.detailRows : [];
  const boxHeight = 92 + detailRows.length * 16;

  doc.setFillColor(255, 255, 255);
  doc.setDrawColor(203, 213, 225);
  doc.roundedRect(marginX, cursorY, boxWidth, boxHeight, 12, 12, "FD");

  doc.setFont("helvetica", "bold");
  doc.setTextColor(100, 116, 139);
  doc.setFontSize(10);
  doc.text(summaryBox.eyebrow || "Summary", marginX + 16, cursorY + 22);

  doc.setFont("helvetica", "bold");
  doc.setTextColor(15, 23, 42);
  doc.setFontSize(22);
  doc.text(summaryBox.primaryValue || "—", marginX + 16, cursorY + 52);

  doc.setFont("helvetica", "normal");
  doc.setFontSize(10);
  doc.setTextColor(71, 85, 105);
  detailRows.forEach((row, index) => {
    const rowY = cursorY + 76 + index * 16;
    doc.text(row.label, marginX + 16, rowY);
...
function finalizePdfPages(doc) {
  ...
  for (let pageNumber = 1; pageNumber <= totalPages; pageNumber += 1) {
    ...
    doc.setFontSize(9);
    doc.setTextColor(100, 116, 139);
    doc.text(`Page ${pageNumber} of ${totalPages}`, pageWidth - marginX, pageHeight - 24, {
      align: "right",
    });
    ...
    if (pageNumber === totalPages) {
      ...
      doc.text("Generated by LedgrAI", marginX, pageHeight - 24);
    }
  }
}

function looksLikePositivePdfAmount(rawValue) {
...
function getPdfDocumentContext() {
  ...
  const activeCompany = getActiveCompany();
  const companyName = company.companyName || activeCompany?.name || "Unnamed Company";
  const companyAddress = [company.address, company.city, company.stateProvince, company.country]
    .filter(Boolean)
    .join(", ");
  ...
  return {
    companyName,
    companyAddress,
    generatedDate: formatLongDate(getTodayDate()),
  };
}
...
function getAssistantAnalysisJournalEntries() {
  return getVisibleJournalEntries();
}

function filterJournalEntriesByDateRange(journalEntries, startDate = "", endDate = "") {
  return (Array.isArray(journalEntries) ? journalEntries : []).filter((entry) => {
...
function buildAssistantMonthRange(offset = 0, referenceDate = new Date()) {
  ...
  return {
    label: offset === 0 ? "Current Month" : offset === -1 ? "Previous Month" : "Month",
    startDate: formatDateInputValue(start),
    endDate: formatDateInputValue(end),
  };
}

function buildAssistantQuarterRange(offset = 0, referenceDate = new Date()) {
  const quarterStartMonth = Math.floor(referenceDate.getMonth() / 3) * 3 + offset * 3;
  ...
  return {
    label: offset === 0 ? "Current Quarter" : offset === -1 ? "Previous Quarter" : "Quarter",
    startDate: formatDateInputValue(start),
    endDate: formatDateInputValue(end),
  };
}

function getDistinctDataMonths(journalEntries) {
  return [...new Set((Array.isArray(journalEntries) ? journalEntries : []).map((entry) => String(entry.date || "").slice(0, 7)).filter(Boolean))].sort();
}

function calculateCashBalanceAtDate(endDate, journalEntries = getAssistantAnalysisJournalEntries()) {
  const entriesThroughDate = filterJournalEntriesByDateRange(journalEntries, "", endDate);
  return state.accounts
    .filter((account) => isCashAccount(account))
    .reduce((sum, account) => {
      const postingTotals = getPostingTotalsForAccount(account.id, entriesThroughDate);
      return (
        sum +
...
function roundAssistantNumber(value) {
  return Math.round((Number(value) || 0) * 100) / 100;
}

function buildAssistantMetricComparison(metric, currentValue, previousValue, { higherIsBetter = true } = {}) {
  const current = roundAssistantNumber(currentValue);
  const previous = roundAssistantNumber(previousValue);
  const changeAmount = roundAssistantNumber(current - previous);
  const changePercentage =
    Math.abs(previous) < 0.005 ? null : roundAssistantNumber((changeAmount / Math.abs(previous)) * 100);
  let assessment = "flat";

  if (Math.abs(changeAmount) >= 0.005) {
    if (Math.abs(previous) < 0.005) {
      assessment = current === 0 ? "flat" : "no baseline";
    } else if (higherIsBetter) {
      assessment = changeAmount > 0 ? "improved" : "declined";
    } else {
      assessment = changeAmount < 0 ? "improved" : "declined";
    }
  }
...
function buildAssistantPeriodMetrics(range, journalEntries = getAssistantAnalysisJournalEntries()) {
  const periodEntries = filterJournalEntriesByDateRange(journalEntries, range.startDate, range.endDate);
  const incomeStatement = buildIncomeStatementReport(periodEntries);
  const balanceSheet = buildBalanceSheetReport(periodEntries);
  const cashFlow = buildCashFlowStatementReport(periodEntries);
  const expenseAccounts = [...incomeStatement.expenseAccounts].sort((left, right) => right.amount - left.amount);
  const revenueAccounts = [...incomeStatement.revenueAccounts].sort((left, right) => right.amount - left.amount);
  const largestExpenseCategory = expenseAccounts[0]
    ? {
        accountName: expenseAccounts[0].account.name,
        amount: roundAssistantNumber(expenseAccounts[0].amount),
        shareOfTotalExpenses:
          incomeStatement.operatingExpenses > 0
            ? roundAssistantNumber((expenseAccounts[0].amount / incomeStatement.operatingExpenses) * 100)
            : null,
      }
    : null;
  ...
  return {
    ...
    totalRevenue: roundAssistantNumber(incomeStatement.totalRevenue),
    totalExpenses: roundAssistantNumber(incomeStatement.operatingExpenses),
    netIncome: roundAssistantNumber(incomeStatement.netIncome),
    closingCashBalance: roundAssistantNumber(calculateCashBalanceAtDate(range.endDate, journalEntries)),
    totalAssets: roundAssistantNumber(balanceSheet.totalAssets),
    totalLiabilities: roundAssistantNumber(balanceSheet.totalLiabilities),
    totalEquity: roundAssistantNumber(balanceSheet.totalEquity),
    operatingCashFlow: roundAssistantNumber(cashFlow.operatingTotal),
    largestExpenseCategory,
    expenseAccounts: expenseAccounts.map((row) => ({
      accountName: row.account.name,
      amount: roundAssistantNumber(row.amount),
    })),
...
function buildAssistantLargestExpenseComparison(currentPeriod, previousPeriod) {
  const currentLargest = currentPeriod.largestExpenseCategory;
  if (!currentLargest) {
    return {
      currentAccountName: null,
      previousAccountName: previousPeriod.largestExpenseCategory?.accountName || null,
      alsoOccurredInPreviousPeriod: false,
      changePercentage: null,
    };
  }

  const previousMatch = previousPeriod.expenseAccounts.find(
    (account) => account.accountName === currentLargest.accountName,
  );

  return {
    currentAccountName: currentLargest.accountName,
    currentAmount: currentLargest.amount,
    previousAccountName: previousPeriod.largestExpenseCategory?.accountName || null,
    previousAmountForSameCategory: previousMatch?.amount ?? 0,
    alsoOccurredInPreviousPeriod:
      previousPeriod.largestExpenseCategory?.accountName === currentLargest.accountName,
    changePercentage:
      previousMatch && Math.abs(previousMatch.amount) >= 0.005
        ? roundAssistantNumber(((currentLargest.amount - previousMatch.amount) / Math.abs(previousMatch.amount)) * 100)
        : null,
  };
}

function buildAssistantPeriodComparison(currentPeriod, previousPeriod, label) {
  return {
    label,
    currentPeriod: {
    ...
    cashPosition: buildAssistantMetricComparison(
      "Cash Position",
      currentPeriod.closingCashBalance,
      previousPeriod.closingCashBalance,
      { higherIsBetter: true },
    ),
    revenue: buildAssistantMetricComparison("Revenue", currentPeriod.totalRevenue, previousPeriod.totalRevenue, {
      higherIsBetter: true,
    }),
    expenses: buildAssistantMetricComparison(
      "Operating Expenses",
      currentPeriod.totalExpenses,
      previousPeriod.totalExpenses,
      { higherIsBetter: false },
    ),
    netIncome: buildAssistantMetricComparison("Net Income", currentPeriod.netIncome, previousPeriod.netIncome, {
      higherIsBetter: true,
    }),
    largestExpenseCategory: buildAssistantLargestExpenseComparison(currentPeriod, previousPeriod),
  };
}

function buildAssistantRevenueSeries(journalEntries = getAssistantAnalysisJournalEntries()) {
  return getDistinctDataMonths(journalEntries)
    .slice(-6)
    .map((monthKey) => {
      const [year, month] = monthKey.split("-").map(Number);
      const startDate = formatDateInputValue(new Date(year, month - 1, 1));
      const endDate = formatDateInputValue(new Date(year, month, 0));
      const metrics = buildAssistantPeriodMetrics(
        {
          label: monthKey,
          startDate,
          endDate,
...
function buildAssistantDataAvailability(journalEntries = getAssistantAnalysisJournalEntries()) {
  const datedEntries = [...journalEntries].filter((entry) => entry.date).sort((left, right) => left.date.localeCompare(right.date));
  const monthsOfData = getDistinctDataMonths(datedEntries).length;

  return {
    monthsOfData,
    firstTransactionDate: datedEntries[0]?.date || "",
    lastTransactionDate: datedEntries[datedEntries.length - 1]?.date || "",
    trendReliabilityNote:
      monthsOfData >= 3
        ? `${monthsOfData} months of data are available, which is enough for directional trend commentary.`
...
function buildAssistantChartOfAccountsTerminology() {
  const accountGroups = {
    assetAccounts: [],
    liabilityAccounts: [],
    equityAccounts: [],
  ...
  state.accounts.forEach((account) => {
    ...
    if (account.type === "Asset") {
      accountGroups.assetAccounts.push(accountLabel);
    } else if (account.type === "Liability") {
      accountGroups.liabilityAccounts.push(accountLabel);
    } else if (account.type === "Equity") {
      accountGroups.equityAccounts.push(accountLabel);
    } else if (account.type === "Revenue") {
      accountGroups.revenueAccounts.push(accountLabel);
    } else if (account.type === "Expense") {
      accountGroups.expenseAccounts.push(accountLabel);
    }
  });

  return {
    ...accountGroups,
    allAccountNames: state.accounts
...
function buildAssistantFinancialContext() {
  ...
  const cashFlow = buildCashFlowStatementReport();
  const activeCompany = getActiveCompany();
  const assistantEntries = getAssistantAnalysisJournalEntries();
  const dataAvailability = buildAssistantDataAvailability(assistantEntries);
  const companyProfile = {
    id: state.companySetup.id,
    companyName: state.companySetup.companyName || activeCompany?.name || "Unnamed Company",
    ...
    monthsOfData: dataAvailability.monthsOfData,
  };
  const currentMonth = buildAssistantPeriodMetrics(buildAssistantMonthRange(0), assistantEntries);
  const previousMonth = buildAssistantPeriodMetrics(buildAssistantMonthRange(-1), assistantEntries);
  const currentQuarter = buildAssistantPeriodMetrics(buildAssistantQuarterRange(0), assistantEntries);
  const previousQuarter = buildAssistantPeriodMetrics(buildAssistantQuarterRange(-1), assistantEntries);
  const monthComparison = buildAssistantPeriodComparison(currentMonth, previousMonth, "Month over Month");
  const quarterComparison = buildAssistantPeriodComparison(currentQuarter, previousQuarter, "Quarter over Quarter");
  const revenueSeries = buildAssistantRevenueSeries(assistantEntries);
  ...
  return {
    ...
    activeCompany: activeCompany
      ...
      : null,
    companyProfile,
    companySetup: companyProfile,
    dataAvailability,
    chartOfAccountsTerminology: buildAssistantChartOfAccountsTerminology(),
    periodAnalysis: {
      currentMonth,
      previousMonth,
      currentQuarter,
    ...
    proactiveInsightsContext: {
      cashPosition: {
        monthComparison: monthComparison.cashPosition,
        quarterComparison: quarterComparison.cashPosition,
      },
      largestExpenseCategory: {
        currentMonth: currentMonth.largestExpenseCategory,
        previousMonth: previousMonth.largestExpenseCategory,
        monthComparison: monthComparison.largestExpenseCategory,
      },
      revenueTrend: {
        monthComparison: monthComparison.revenue,
        quarterComparison: quarterComparison.revenue,
        sufficientData: dataAvailability.monthsOfData >= 3,
      },
      anomalyContext: {
        currentMonthLargestExpenseAlsoOccurredLastMonth:
          monthComparison.largestExpenseCategory.alsoOccurredInPreviousPeriod,
        currentQuarterLargestExpenseAlsoOccurredLastQuarter:
          quarterComparison.largestExpenseCategory.alsoOccurredInPreviousPeriod,
      },
    },
    ...
    journalEntries: assistantEntries.map((entry) => ({
      date: entry.date,
      description: entry.description,
    ...
    workingCapital: {
      outstandingInvoicesTotal: roundAssistantNumber(getOutstandingInvoicesTotal()),
      overdueInvoicesCount: getOverdueInvoicesCount(),
      outstandingBillsTotal: roundAssistantNumber(getOutstandingBillsTotal()),
      overdueBillsCount: getOverdueBillsCount(),
    },
    trialBalance: {
